...
% Enforce Dirichlet boundary conditions on initial condition
grf.aAmplitude = zeros(size(grf.aAmplitude));
...
if datasetType == "inspect"
    ...
    datasetSize = 1;
    randomSeed = 1234;
    datasetName = "inspectClassical4.ol.h5";
elseif datasetType == "test"
    numChunks = 32;
    datasetSize = 500;
    randomSeed = 1234;
    datasetName = "testClassical4.ol.h5";
elseif datasetType == "train"
    numChunks = 32;
    datasetSize = 3000;
    randomSeed = 2026;
    datasetName = "trainClassical4.ol.h5";
end
...
function generator = MakeGenerator(numModes, alpha, beta, gamma, Xo, Xn, Yo, Yn)
    function GenerateOne(simulation, outputFile, ~, sampleIndex, curStream)
        grf = GRF2DON(numModes, @(i, j) alpha ./ ((beta + i.^2 + j.^2) .^ (gamma/2)), [Xo, Yo, 2*Xn - Xo, 2*Yn - Yo], curStream);
        % Enforce Dirichlet boundary conditions on initial condition
        grf.aAmplitude = zeros(size(grf.aAmplitude));
        grf.bAmplitude = zeros(size(grf.bAmplitude));
        grf.cAmplitude = zeros(size(grf.cAmplitude));
