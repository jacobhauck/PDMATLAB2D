...
datasetName = "sym.ol.h5";

xy = [sim.xx, sim.yy];
generator = MakeGenerator(datasetSize, numChunks, sigmaRange, Yo, Yn, dy, sigma, bx, PreNotchCoordinates);
GenerateDataset(datasetName, datasetSize, numChunks, seed, sim, bx, xy, 2, 1, generator);

function generator = MakeGenerator(datasetSize, numChunks, sigmaRange, Yo, Yn, dy, sigma, bx, PreNotchCoordinates)
    function GenerateOne(simulation, outputFile, chunkIndex, sampleIndex, ~)
        ...
        sigmaRel = t * (sigmaRange(2) - sigmaRange(1)) + sigmaRange(1);
        modtop = @(x) ones(size(x)) * sigmaRel;
        modbot = @(x) ones(size(x)) * sigmaRel;
